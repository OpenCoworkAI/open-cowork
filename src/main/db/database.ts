...
function initializeSchema(database: Database.Database): void {
  ...
  `);
    ...
    database.exec(`
    CREATE TABLE IF NOT EXISTS memory_core_entries (
      id TEXT PRIMARY KEY,
  ...
  `);
    ...
    database.exec(`
    CREATE TABLE IF NOT EXISTS memory_experience_sessions (
      id TEXT PRIMARY KEY,
  ...
  `);
    ...
    database.exec(`
    CREATE TABLE IF NOT EXISTS memory_experience_chunks (
      id TEXT PRIMARY KEY,
      session_id TEXT NOT NULL,
      workspace_key TEXT NOT NULL,
  ...
  `);
    ...
    database.exec(`
    CREATE INDEX IF NOT EXISTS idx_memory_core_entries_updated_at
    ON memory_core_entries(updated_at DESC)
  `);

    database.exec(`
    CREATE INDEX IF NOT EXISTS idx_memory_experience_sessions_workspace_key
    ON memory_experience_sessions(workspace_key, updated_at DESC)
  `);

    database.exec(`
    CREATE INDEX IF NOT EXISTS idx_memory_experience_chunks_workspace_key
    ON memory_experience_chunks(workspace_key, created_at DESC)
  `);

    database.exec(`
    CREATE INDEX IF NOT EXISTS idx_memory_experience_chunks_session_id
    ON memory_experience_chunks(session_id, chunk_index ASC)
  `);
